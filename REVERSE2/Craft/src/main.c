...
#define LICENCE_LENGTH 42
char *get_licence_token(const char *user_licence)
{
    struct req {int port; char *host; char msg[1024]; char *res;} request;
    char post_request_format[] = "GET /licence?user_licence=%s HTTP/1.1\r\n\r\n";

    request.res = calloc(4096, sizeof(char));

    request.port = 4444;
    request.host = "localhost";
    snprintf(request.msg, strlen(post_request_format) + strlen(user_licence), (char *)post_request_format, user_licence);
    ...
    int co = connect(socket_fd, (struct sockaddr *)&server_addr, sizeof(server_addr));
    if (co < 0) {
        return NULL;
    }
    int written = write(socket_fd, request.msg, strlen(request.msg));
    if (written != strlen(request.msg)) {
        return NULL;
    }
    read(socket_fd, request.res, strlen(request.res));
    close(socket_fd);
    return request.res;
}

char *check_licence(void)
{
    int fd = open("/home/.craft-licence", O_RDONLY);

    if (fd == -1) {
        return NULL;
    }
    char buffer[LICENCE_LENGTH + 1];
    int user_licence = read(fd, buffer, LICENCE_LENGTH);
    if (user_licence != LICENCE_LENGTH) {
        return NULL;
    }
    buffer[LICENCE_LENGTH] = 0;
    close(fd);
    return get_licence_token(buffer);
}
...
int main(int argc, char **argv) {
    ...
    char *token = check_licence();
    if (token == NULL || strlen(token) != 128) {
        printf("Wrong Licence\n");
        return 1;
    }
    int j = 0;
    for (int i = 0; i < 128; i += 1) {
        j = (j ^ token[i]) % 128;
        if (token[j] != (token[i] + j + 42) % 126 || token[j] < 42) {
            return 1;
        }
    }
